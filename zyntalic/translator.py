...
def translate_sentence(
    ...
    *,
    ...
    if engine == "test_suite":
        try:
            # Translation itself runs on the core engine; the row is only tagged with
            # validation metadata. Constructing ZyntalicTestSuite here would spend
            # minutes building an unused lexicon on every call, so it is not built.
